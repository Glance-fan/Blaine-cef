var ActionBox = class ActionBox {
    ...
    static fill(needBG, type, fulltext, data) {
        var box_data = this.getBoxData(type);
        actionbox.firstElementChild.children[0].innerHTML = box_data[0] + fulltext;
        actionbox.firstElementChild.children[1].style.height = box_data[1][0];
        actionbox.firstElementChild.style.height = box_data[1][1];
        setTimeout(() => {
            actionbox.style.top = `calc(50% - ${actionbox.offsetHeight/2}px)`
        }, 0)
        this.drawContainer(type, data);
    ...
    static getBoxData(which) {
        switch (which) {
            case 0: //choice
                return [actionbox_svgs.finger, ['400px', '460px']];
            case 1: //input
                return [actionbox_svgs.input, ['400px', '460px']];
            case 2: //range
                return [actionbox_svgs.finger, ['250px', '310px']];
            case 3: //money
                return [hud_left_svgs.cash, ['400px', '460px']];
        }
    }

    static container = document.getElementById('action-box-container');
    static drawContainer(which, data) {
        switch (which) {
            case 0: //choice
                ...
                break;
            case 1: //input
                this.container.innerHTML = /*html*/ `<textarea autocomplete="off" spellcheck="false" oninput="ActionBox.curData = this.value" maxlength="${data[0] || 250}" class="action-box-input" placeholder="Введите текст..."></textarea>`
                if (data[1])
                    setTimeout(() => {
            ...
            this.container.parentElement.style.top = `calc(50% - 27px)`;
                ...
                <div class="action-box-span-wrapper">
                    ...
                    <span style="text-align: right;">${data[1]}</span>
                </div>
                `;
                ActionBox.onrange(this.container.firstElementChild, document.getElementById('action-box-range'));
                break;
            case 3: //money
                this.container.innerHTML = /*html*/ `<div id="actionbox-money-text">${data[0]}</div> `;
...
//ActionBox.fill(true, 0, 'choice-actionbox', [[17, 'maxblack'], [14, 'jeka'], [13, 'jeka'], [12, 'jeka'], [11, 'jeka']])
//ActionBox.fill(false, 1, 'input-actionbox', [250, 'default-text'])
//ActionBox.fill(true, 2, 'range-actionbox', [1, 4000, 1])
//ActionBox.fill(false, 3., 'money-actionbox', ['TEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERETEXTHERE'])
