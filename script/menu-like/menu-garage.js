var MenuGar = class MenuGarage {
    ...
    static selectGarage(el) {
        ...
        this.fillGarInfo(parseInt(el.id), [el.getAttribute('capacity'), el.getAttribute('owner'), this.prettyUSD(el.getAttribute('cost')), this.prettyUSD(el.getAttribute('tax'), true)]);
    ...
    static fillGarInfo(id, data) {
        ...
        for (var index = 0; index < this.static_info.length; index++) {
            info.firstElementChild.innerHTML += /*html*/
                `<div style="font-weight: 500">${this.static_info[index]}</div>`
    ...
    static fillButtons(el) {
        ...
        container.innerHTML = /*html*/ `
            ...
            <div style="justify-content: center;">
                ${el.getAttribute('locked') === 'false' || id == this.personalId ? /*html*/ `<button class="red-button" onclick="MenuGar.onbutton('enter',  ${id})">Войти</button>`: ``}
            </div>
            ${id == this.personalId ? /*html*/ `
            <div>
